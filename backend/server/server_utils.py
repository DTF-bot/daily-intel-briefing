...
def get_missing_research_credentials(report_source: str) -> list[str]:
    missing = []
    ...
    if "deepseek" in llm_providers and not os.getenv("DEEPSEEK_API_KEY"):
        missing.append("DEEPSEEK_API_KEY")
    if "openai" in llm_providers and not os.getenv("OPENAI_API_KEY") and not os.getenv("OPENAI_ADMIN_KEY"):
        missing.append("OPENAI_API_KEY")
    if config.embedding_provider == "openai" and not os.getenv("OPENAI_API_KEY"):
        missing.append("OPENAI_API_KEY（用于 embedding）")

    retrievers = os.getenv("RETRIEVER", "tavily").split(",")
    uses_web_search = report_source in {"web", "hybrid"} or not report_source
    uses_tavily = any(retriever.strip().lower() == "tavily" for retriever in retrievers)
    if uses_web_search and uses_tavily and not os.getenv("TAVILY_API_KEY"):
        missing.append("TAVILY_API_KEY")

    return missing


def get_research_credentials_error(report_source: str) -> str:
    from gpt_researcher.config.config import Config

    config = Config()
    missing = get_missing_research_credentials(report_source)
    errors = []
    if missing:
        errors.append(
            "缺少运行每日情报速递采集所需配置："
            + "、".join(missing)
            + "。请在项目根目录创建 .env 并配置后重启服务。"
        )
    ...
    if "deepseek" in llm_providers and os.getenv("DEEPSEEK_API_KEY"):
        try:
            ...
            if "Insufficient Balance" in error_text or "402" in error_text:
                errors.append("DeepSeek API 余额不足，请在 DeepSeek 控制台充值或更换有余额的 key。")
            else:
                errors.append(f"无法验证 DEEPSEEK_API_KEY：{e}。请检查 DeepSeek key、网络或代理配置。")

    if "openai" in llm_providers and os.getenv("OPENAI_API_KEY"):
        try:
            ...
            errors.append("OPENAI_API_KEY 无效。请到 OpenAI Platform 重新创建有效 API key，更新 .env 后重启服务。")
        except Exception as e:
            errors.append(f"无法验证 OPENAI_API_KEY：{e}。请检查网络、代理或 OpenAI API 配置。")

    if (
        config.embedding_provider == "openai"
        and "deepseek" in llm_providers
        and os.getenv("OPENAI_API_KEY")
        and os.getenv("OPENAI_API_KEY") == os.getenv("DEEPSEEK_API_KEY")
    ):
        errors.append(
            "当前 EMBEDDING 仍是 OpenAI，但 OPENAI_API_KEY 和 DEEPSEEK_API_KEY 相同。"
            "DeepSeek key 不能用于 OpenAI embedding；请配置有效 OpenAI key 用于 embedding，"
            "或改用本地/HuggingFace embedding。"
        )
...
async def handle_start_command(websocket, data: str, manager):
    ...
    if not task or not report_type:
        ...
        await websocket.send_json({
            "type": "error",
            "content": "error",
            "output": "任务内容或报告类型缺失，请填写情报主题后重新开始。",
        })
        return

    credentials_error = get_research_credentials_error(report_source)
    if credentials_error:
        await websocket.send_json({
            "type": "error",
            "content": "error",
            "output": credentials_error,
        })
        return
