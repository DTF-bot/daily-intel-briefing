...
from server.daily_intel import (
    DailyIntelJobInput,
    build_report_detail_html,
    public_job,
    run_daily_intel_job,
    start_daily_intel_job,
    start_scheduler,
    stop_scheduler,
    store as daily_intel_store,
)
...
async def lifespan(app: FastAPI):
    ...
    start_scheduler()
    logger.info("Daily Intel Briefing API ready - scheduler enabled in local mode")
    yield
    # Shutdown
...
async def serve_frontend():
    ...
    return HTMLResponse(content=read_frontend_index())


@app.get("/config", response_class=HTMLResponse)
async def serve_config_page():
    """Serve the daily intel configuration page."""
    return HTMLResponse(content=read_frontend_index())
...
@app.get("/api/daily-intel/jobs")
async def list_daily_intel_jobs():
    return {"jobs": [public_job(job) for job in await daily_intel_store.list_jobs()]}


@app.post("/api/daily-intel/jobs")
async def create_daily_intel_job(job: DailyIntelJobInput):
    return {"job": public_job(await daily_intel_store.upsert_job(job))}


@app.put("/api/daily-intel/jobs/{job_id}")
async def update_daily_intel_job(job_id: str, job: DailyIntelJobInput):
    existing = await daily_intel_store.get_job(job_id)
    if not existing:
        raise HTTPException(status_code=404, detail="Daily intel job not found")
    return {"job": public_job(await daily_intel_store.upsert_job(job, job_id=job_id))}


@app.delete("/api/daily-intel/jobs/{job_id}")
async def delete_daily_intel_job(job_id: str):
    deleted = await daily_intel_store.delete_job(job_id)
    if not deleted:
        raise HTTPException(status_code=404, detail="Daily intel job not found")
    return {"success": True}


@app.post("/api/daily-intel/jobs/{job_id}/run")
async def run_daily_intel_job_now(job_id: str):
    return {"run": await start_daily_intel_job(job_id)}


@app.get("/api/daily-intel/runs")
async def list_daily_intel_runs():
    return {"runs": await daily_intel_store.list_runs()}


@app.get("/api/daily-intel/runs/{run_id}")
async def get_daily_intel_run(run_id: str):
    run = await daily_intel_store.get_run(run_id)
    if not run:
        raise HTTPException(status_code=404, detail="Daily intel run not found")
    return {"run": run}


@app.get("/daily-intel/report/{run_id}", response_class=HTMLResponse)
async def daily_intel_report_detail(run_id: str):
    return HTMLResponse(content=await build_report_detail_html(run_id))
