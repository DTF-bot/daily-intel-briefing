...
class LocalHashEmbeddings(Embeddings):
    """Small deterministic embedding fallback for local development.

    This avoids external embedding APIs when an OpenAI-compatible gateway only
    supports chat models. It is not as semantically rich as a real embedding
    model, but it is enough to keep the research pipeline running.
    """
    ...
    def _tokens(text: str) -> list[str]:
        ...
        char_grams = [
            ...
            if not text[index:index + 3].isspace()
        ]
    ...
    def __init__(self, embedding_provider: str, model: str, **embedding_kwargs: Any):
        ...
        match embedding_provider:
            case "local":
                ...
                _embeddings = LocalHashEmbeddings(dimensions=dimensions)
            case "custom":
